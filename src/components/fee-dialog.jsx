...
export function FeeDialog({ open, onOpenChange, txn }) {
  ...
  const save = () => {
    if (!f.student || f.amount <= 0)
      return toast.error("Student and amount are required");
    if (txn) {
      feeApi.update(txn.id, f);
      toast.success("Transaction updated");
    } else {
      feeApi.add(f);
      toast.success(`Receipt issued — ₹${f.amount.toLocaleString("en-IN")}`);
    }
    onOpenChange(false);
  };
  return (
      ...
      <DialogContent className="max-w-xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-2">
          <Field label="Student name">
            <Input
              value={f.student}
              onChange={(e) => setF({ ...f, student: e.target.value })}
          ...
          <Field label="Class">
            <Input
              value={f.class}
              onChange={(e) => setF({ ...f, class: e.target.value })}
              placeholder="X-B"
            />
          </Field>
          ...
          <Field label="Amount (₹)">
            <Input
              ...
              value={f.amount}
              onChange={(e) =>
                setF({ ...f, amount: parseInt(e.target.value) || 0 })
              }
            />
          </Field>
