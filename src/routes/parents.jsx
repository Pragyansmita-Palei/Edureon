  ...
  const parents = useMemo(() => {
    ...
    students.forEach((s, i) => {
      ...
      if (!map.has(key)) {
        map.set(key, {
          ...
          children: [],
          lastLogin: ["2h ago", "Yesterday", "3d ago", "1w ago", "Never"][
            i % 5
          ],
        });
      }
    ...
    return Array.from(map.values()).filter(
      (p) =>
        ...
        p.children.some((c) => c.toLowerCase().includes(q.toLowerCase())),
    );
  }, [students, q]);
  return (
    <PageContainer>
        ...
        actions={
          <>
            <Button
              variant="outline"
              size="sm"
              onClick={() => toast.success("Bulk SMS queued · 4,820 parents")}
            >
              ...
              {parents.slice(0, 24).map((p) => (
                <TableRow key={p.name + p.phone}>
                  ...
                  <TableCell className="text-xs">
                    <Badge
                      variant={
                        p.lastLogin === "Never" ? "destructive" : "outline"
                      }
                    >
                      {p.lastLogin}
                    </Badge>
