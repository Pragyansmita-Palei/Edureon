...
export default function ChildProfile() {
    ...
    return (
      <PageContainer>
        <div className="text-sm text-muted-foreground">
          Not found.{" "}
          <Link to="/parent/children" className="underline">
            Back
          </Link>
        </div>
      </PageContainer>
  ...
  const myMarks = marks.filter(
    (m) => m.studentId === child.id && m.status === "Published",
  );
  ...
  const submitLeave = () => {
    ...
    setOpen(false);
    setForm({ from: "", to: "", reason: "", type: "Sick" });
  };
  return (
    <PageContainer>
      <PageHeader
        eyebrow={child.admissionNo}
        title={child.name}
      ...
      <Tabs defaultValue="overview">
          ...
          <Card>
            <CardContent className="p-4 text-sm space-y-2">
              ...
              <div className="flex justify-between border-b py-1.5">
                ...
                <Badge
                  variant={child.feeStatus === "Paid" ? "default" : "outline"}
                >
                  {child.feeStatus}
                </Badge>
              </div>
          ...
          <Card>
            <CardContent className="p-0 divide-y">
              {myAssignments.map((a) => {
                ...
                return (
                  <div key={a.id} className="p-2.5 flex items-center gap-2">
            ...
            <CardContent className="p-0 divide-y">
              {myMarks.map((m) => (
                <div key={m.id} className="p-2.5 flex items-center gap-2">
                  <span className="text-sm flex-1">{m.subject}</span>
                  <Badge variant="outline" className="text-[10px]">
                    {m.klass}
                  </Badge>
        ...
        <TabsContent value="exams">
          <Card>
            <CardContent className="p-0 divide-y">
              {upExams.map((e) => (
                <div key={e.id} className="p-2.5 flex items-center gap-2">
            ...
            <CardContent className="p-0 divide-y">
              {myNotices.map((n) => (
                <div key={n.id} className="p-2.5">
                  ...
                  <div className="text-[11px] mt-0.5">{n.body}</div>
                </div>
              ))}
            </CardContent>
          </Card>
