        ...
        actions={
          <>
            ...
            <Button
              ...
              size="sm"
              onClick={() =>
                toast.info("Edit from Classes › Subjects → row menu")
              }
            >
            ...
            <Button
              ...
              size="sm"
              onClick={() => {
                subjectsApi.archive(sub.id, !sub.archived);
                toast.success(sub.archived ? "Restored" : "Archived");
              }}
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <Card className="border-border/60">
          <CardContent className="p-4">
            ...
            <div className="text-xl font-semibold mt-1">{sub.type}</div>
          </CardContent>
        </Card>
        <Card className="border-border/60">
          <CardContent className="p-4">
            <div className="text-xs text-muted-foreground">Classes</div>
            <div className="text-xl font-semibold mt-1">{subMaps.length}</div>
          </CardContent>
        </Card>
        <Card className="border-border/60">
          <CardContent className="p-4">
        ...
        <Card className="border-border/60">
          <CardContent className="p-4">
            ...
            <div className="text-xl font-semibold mt-1">
              {subMaps.reduce((a, b) => a + b.periods, 0)}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="classes">
              ...
              {teacherSet.map((t) => (
                ...
                >
                  ...
                  <Badge variant="outline">
                    {subMaps.filter((m) => m.teacher === t).length} sections
                  </Badge>
                </div>
              ))}
            </CardContent>
          ...
          <Card className="border-border/60">
            <CardContent className="p-6 text-sm flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Assignments tagged with {sub.name}
                </CardDescription>
              </div>
        ...
        <TabsContent value="exams" className="mt-4">
          <Card className="border-border/60">
            <CardContent className="p-6 text-sm flex items-center justify-between">
              <div>
                <CardTitle className="text-base">Linked exams</CardTitle>
                <CardDescription>
                  Exam papers and results for {sub.name}
                </CardDescription>
              </div>
              <Button asChild variant="outline" size="sm">
          ...
          <Card className="border-border/60">
            <CardContent className="p-6 text-sm flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Chapter-wise plans for {sub.name}
                </CardDescription>
              </div>
              <Button asChild variant="outline" size="sm">
                <Link to="/teacher/lesson-plans">Open Lesson Plans →</Link>
              </Button>
            </CardContent>
          </Card>
        </TabsContent>
