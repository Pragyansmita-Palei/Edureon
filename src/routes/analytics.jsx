...
export default function AnalyticsPage() {
  ...
  const attendance = attendanceTrend.map((d) => ({
    ...
    students: Math.min(
      100,
      Math.round(d.students * (0.85 + (mult - 1) * 0.15)),
    ),
  ...
  const exams = examPerformance.map((d) => ({
    ...
    top: Math.min(
      100,
      Math.round((d.top ?? d.avg + 10) * (0.9 + (mult - 1) * 0.15)),
    ),
  }));
        ...
        actions={
          <>
            <Button
              ...
              size="sm"
              onClick={() =>
                toast.success("Export scheduled — emailed in 5 min")
              }
            >
            ...
            <Button
              ...
              onClick={() => toast.success("New report draft saved")}
            >
      ...
      <Card className="border-border/60 mb-6">
        <CardContent className="p-3 flex flex-wrap items-center gap-2">
          ...
          <Badge variant="outline" className="ml-auto text-[10px]">
            {inst === "all"
              ? "All Institutes"
              : institutes.find((i) => i.id === inst)?.name}{" "}
            · {range.toUpperCase()}
          </Badge>
        </CardContent>
      </Card>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Click a widget on the left to add it to your dashboard
                  </CardDescription>
                </div>
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      toast.success(
                        `Report saved · ${widgets.length} widget${widgets.length === 1 ? "" : "s"}`,
                      )
                    }
                  >
                    Save Report
...
function BuilderWidget({ w, onRemove }) {
  return (
    <div className="border rounded-md p-3 bg-background relative group">
      <button
        onClick={onRemove}
        className="absolute top-2 right-2 h-6 w-6 rounded flex items-center justify-center opacity-0 group-hover:opacity-100 hover:bg-destructive/10 text-destructive transition"
      >
      ...
      {w.type === "line" && (
        <ResponsiveContainer width="100%" height={120}>
          <LineChart data={attendanceTrend}>
            ...
            <YAxis domain={[80, 100]} fontSize={9} hide />
          </LineChart>
        </ResponsiveContainer>
      )}
      {w.type === "bar" && (
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="text-muted-foreground">
              ...
              <th className="text-right font-normal">Students</th>
            </tr>
          </thead>
          <tbody>
            {classDistribution.slice(0, 4).map((c) => (
              <tr key={c.class} className="border-t">
