...
export default function ParentDashboard() {
  ...
  const childSubs = useMemo(
    () => subs.filter((s) => s.studentId === CHILD_ID),
    [subs],
  );
  const pending = childAssignments.filter(
    (a) =>
      !childSubs.find((s) => s.assignmentId === a.id) ||
      childSubs.find((s) => s.assignmentId === a.id)?.status === "Pending",
  ).length;
  const upExams = exams.filter(
    (e) =>
      e.class === child?.class &&
      (e.status === "Scheduled" || e.status === "In Progress"),
  );
  const recentMarks = marks
    .filter((m) => m.studentId === CHILD_ID && m.status === "Published")
    .slice(0, 4);
  ...
  return (
    <PageContainer>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <KpiCard
          label="Attendance"
          value={`${child?.attendance ?? 93}%`}
          icon={<CalendarCheck className="h-5 w-5" />}
          tone={attAlert ? "warning" : "success"}
        />
        <KpiCard
      ...
      {attAlert && (
        <div className="mb-4 p-3 rounded-md border border-warning/30 bg-warning/10 text-warning text-sm">
          ⚠️ Attendance dropped below 85%. Please review with your child.
        </div>
      )}
        ...
        <Card>
          ...
          <CardContent className="space-y-1">
            {childAssignments.slice(0, 5).map((a) => {
              const s = childSubs.find((x) => x.assignmentId === a.id);
              return (
                <div
                  key={a.id}
