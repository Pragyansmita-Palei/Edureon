          ...
          <CardContent className="p-5 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Button
                ...
                variant="outline"
                onClick={() => {
                  setComm({ channel: "Email", subject: "", body: "" });
                  setCommOpen(true);
                }}
              >
              ...
              <Button
                ...
                variant="outline"
                onClick={() => {
                  setComm({ channel: "SMS", subject: "Update", body: "" });
                  setCommOpen(true);
                }}
              >
              ...
              <Button
                size="sm"
                variant="outline"
                onClick={() => toast.success(`Calling ${inq.phone}…`)}
              >
              ...
              <Button
                ...
                variant="outline"
                onClick={() => {
                  setComm({ channel: "WhatsApp", subject: "Update", body: "" });
                  setCommOpen(true);
                }}
              >
                <MessageSquare className="h-3.5 w-3.5" />
            ...
            <div className="pt-2 border-t">
              ...
              <Select
                value={inq.counselor}
                onValueChange={(v) => {
                  inquiriesApi.assignCounselor(id, v);
              ...
              >
                ...
                <SelectContent>
                  {["Sneha K.", "Rohit M.", "Priya S.", "Vikram T."].map(
                    (c) => (
          ...
          <Card>
            <CardContent className="p-5 grid md:grid-cols-2 gap-4">
              <Field
                label="Full name"
                value={inq.name}
                onSave={(v) => inquiriesApi.update(id, { name: v })}
              />
              <Field
                label="Applying for class"
                value={inq.class}
                onSave={(v) => inquiriesApi.update(id, { class: v })}
              />
              <Field
                label="Date of birth"
                value={inq.dob || ""}
                type="date"
                onSave={(v) => inquiriesApi.update(id, { dob: v })}
              />
              <Field
                label="Gender"
                value={inq.gender || "Male"}
                onSave={(v) => inquiriesApi.update(id, { gender: v })}
              />
              <Field
                label="Previous school"
                value={inq.prevSchool || ""}
                onSave={(v) => inquiriesApi.update(id, { prevSchool: v })}
              />
              <Field
                label="Address"
                value={inq.address || ""}
                onSave={(v) => inquiriesApi.update(id, { address: v })}
              />
            </CardContent>
          </Card>
        </TabsContent>
          ...
          <Card>
            <CardContent className="p-5 grid md:grid-cols-2 gap-4">
              <Field
                ...
                value={inq.parent}
                onSave={(v) => inquiriesApi.update(id, { parent: v })}
              />
              <Field
                label="Mother"
                value={inq.motherName || ""}
                onSave={(v) => inquiriesApi.update(id, { motherName: v })}
              />
              <Field
                label="Phone"
                value={inq.phone}
                onSave={(v) => inquiriesApi.update(id, { phone: v })}
              />
              <Field
                label="Email"
                value={inq.email}
                onSave={(v) => inquiriesApi.update(id, { email: v })}
              />
            </CardContent>
          </Card>
        </TabsContent>
            ...
            <CardContent className="p-5 pt-0 space-y-3">
              <div className="grid md:grid-cols-3 gap-2">
                <Input
                  type="date"
                  value={fu.due}
                  onChange={(e) => setFu({ ...fu, due: e.target.value })}
                />
                <Input
                  className="md:col-span-2"
                  placeholder="Note (e.g. discuss scholarship)"
                  value={fu.note}
                  onChange={(e) => setFu({ ...fu, note: e.target.value })}
                />
              </div>
              <Button
                size="sm"
                ...
                onClick={() => {
                  inquiriesApi.addFollowUp(id, fu.due, fu.note);
                  setFu({ due: "", note: "" });
                ...
                {inq.followUps.map((f) => (
                  <div key={f.id} className="flex items-center gap-3 p-3">
                    <Checkbox
                      checked={f.done}
                      onCheckedChange={() =>
                        inquiriesApi.toggleFollowUp(id, f.id)
                      }
                    />
                    <div className="flex-1">
                      <div
                        className={`text-sm ${f.done ? "line-through text-muted-foreground" : ""}`}
                      >
                        {f.note}
                      </div>
                  ...
                  {inq.history.map((h, i) => (
                    <div key={i} className="flex items-center gap-2 text-xs">
                      ...
                      <span className="text-muted-foreground">
                        {new Date(h.at).toLocaleString()} · by {h.by}
                      </span>
                    </div>
                  ))}
                </div>
            ...
            <CardContent className="p-5 space-y-2">
              {(inq.documents || []).map((d) => (
                ...
                >
                  <Checkbox
                    checked={d.ok}
                    onCheckedChange={() => inquiriesApi.toggleDoc(id, d.name)}
                  />
                  <FileCheck2
                    className={`h-4 w-4 ${d.ok ? "text-success" : "text-muted-foreground"}`}
                  />
                  <div className="flex-1 text-sm">{d.name}</div>
                  <Badge
                    variant={d.ok ? "default" : "outline"}
                    className="text-[10px]"
                  >
                    {d.ok ? "Verified" : "Pending"}
                  </Badge>
                  <Button
                    size="sm"
                    variant="ghost"
            ...
            <CardContent className="p-5 space-y-3">
              <div className="flex gap-2">
                ...
                <Button
                  onClick={() => {
                    if (noteText.trim()) {
                      notesApi.add("inquiry", id, noteText);
                      setNoteText("");
              ...
              {commOpen && (
                <div className="p-3 border rounded-md space-y-2 bg-muted/30">
                  <div className="grid grid-cols-3 gap-2">
                    <Select
                      value={comm.channel}
                      onValueChange={(v) => setComm({ ...comm, channel: v })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        {["Email", "SMS", "WhatsApp", "Call"].map((c) => (
                          <SelectItem key={c} value={c}>
                            {c}
                          </SelectItem>
                        ))}
                    ...
                    <Input
                      ...
                      value={comm.subject}
                      onChange={(e) =>
                        setComm({ ...comm, subject: e.target.value })
                      }
                    />
                  </div>
                  <Textarea
                  ...
                  <div className="flex gap-2 justify-end">
                    ...
                    <Button
                      size="sm"
                      disabled={!comm.subject || !comm.body}
                      onClick={() => {
                        inquiriesApi.addComm(id, comm);
                        setCommOpen(false);
                        toast.success(`${comm.channel} sent`);
                      }}
                    >
            ...
            <CardContent className="p-5 space-y-3">
              <div className="grid md:grid-cols-3 gap-3">
                <Stat
                  ...
                  label="Fee total"
                  value={`₹${(inq.feeTotal || 0).toLocaleString("en-IN")}`}
                />
                <Stat
                  icon={<IndianRupee className="h-4 w-4" />}
                  label="Paid"
                  value={`₹${(inq.feePaid || 0).toLocaleString("en-IN")}`}
                />
                <Stat
                  icon={<IndianRupee className="h-4 w-4" />}
                  label="Balance"
                  value={`₹${((inq.feeTotal || 0) - (inq.feePaid || 0)).toLocaleString("en-IN")}`}
                />
              </div>
              <Progress
                value={Math.round(
                  ((inq.feePaid || 0) / (inq.feeTotal || 1)) * 100,
                )}
              />
              <div className="flex gap-2">
                ...
                <Button
                  onClick={() => {
                    ...
                    if (v > 0) {
                      inquiriesApi.update(id, {
                        feePaid: (inq.feePaid || 0) + v,
                      });
                      el.value = "";
                      toast.success(`₹${v.toLocaleString("en-IN")} collected`);
                    }
                  }}
                >
